...
class FirebaseTokenVerifier private constructor(
    ...
    override fun verify(token: String): FirebasePrincipal? {
    ...
    companion object {
        fun create(config: AppConfig): FirebaseTokenVerifier {
            val path = config.googleApplicationCredentials?.trim().orEmpty()
            if (path.isEmpty()) {
                LoggerFactory.getLogger(FirebaseTokenVerifier::class.java)
                    .info("GOOGLE_APPLICATION_CREDENTIALS not set; Firebase token verification is disabled.")
                return FirebaseTokenVerifier(firebaseAuth = null)
            }
